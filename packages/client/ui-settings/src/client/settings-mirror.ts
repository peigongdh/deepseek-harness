...
export interface SettingsMirrorSnapshot {
  /**
   * `unavailable` is the terminal memory-persistence state; `ready` persists across
...
export class SettingsDescribeMirror implements SettingsDescribeFace {
   ...
   * @param persistence - client-selected Host persistence; explicit memory consumers remain process-local.
   */
  constructor(
