...
describe('ui-theme apply', () => {
  ...
  it('loads Host settings at boot, refreshes its namespace, and persists remote browser preferences', async () => {
    const b = await bench()
    ...
    expect(remote.mutate).toHaveBeenCalledWith(
      THEME_SETTINGS_NAMESPACE, [{ op: 'set', path: ['preference'], value: 'dark' }], 0,
    )
  })
