...
The request header defaults to two lines with a full-text disclosure. Wide layouts allocate user/context, model, and tools/mechanisms in a 1:1:3 ratio. Workflow runs and Agent dispatch batches summarize each member in one row; Plan mode, Goal revisions, and Todo changes retain their own presentations. Narrow containers stack cards. Capability labels distinguish files/environment, search, skills, human interaction, and deliverables. Unknown tools keep their recorded name and generic category.

Key member rows remain visible, with raw scheduling events folded underneath. Workflow marks align member start/end event order without scaling by duration. Complete pairs establish observed overlap or non-overlap, not dependencies or waiting barriers. Agent rows show parent-observed dispatch and receipts; receipt delivery does not establish execution end or concurrency. Later messages, settlement notices, and controls remain at their original positions as compact receipts with Locate dispatch and Return to receipt actions. Missing origins are labeled. Todo compares whole-list counts without inferring task identities. Select a lifecycle endpoint or raw card to inspect its original record.

Expand a recorded child to read its history below the complete scheduling block inside the parent request. Expansion follows the authenticated child address without selecting or resuming its Agent; collapse releases the view observation without canceling work. Child pages and failures remain local to that child. Inherited fork records are labeled when their recorded seed boundary is loaded. The view displays recorded evidence and does not verify the Agent’s conclusions.
