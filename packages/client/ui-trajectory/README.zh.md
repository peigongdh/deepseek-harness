...
中间按顺序向下排列用户／上下文、模型和工具卡片。能力标签区分文件／环境操作、搜索、技能、计划／目标、Agent 协作、工作流、人机交互及成果交付。连续常规步骤折叠为带数量的展开项；错误和机制状态变更保持可见。选中卡片可检查原文、参数、结果、Schema 或计时。未知工具保留记录中的名称及通用类别。

展开已记录的子任务，可在父请求内阅读子历史。展开通过认证的子会话地址跟随历史，不选择或恢复其 Agent；收起只释放视图观察，不取消工作。子会话的分页和失败独立处理。加载到已记录的种子边界时，fork 继承内容会明确标记。视图展示记录证据，不验证 Agent 的结论。
