...
打开“增强轨迹”，可跨执行轮次追踪一次用户请求。左侧列表区分直接用户请求，将回答、运行中引导和自动续轮留在所属请求内。手动压缩显示在请求之间。未加载的历史有明确提示；“加载更早历史”扩展同一请求列表。

请求标题默认显示两行，可展开全文。宽屏按 1:1:3 为用户／上下文、模型、工具／机制分配空间。Workflow 运行和 Agent 派发批次将每个成员汇总为一行；Plan 模式、Goal 修订和 Todo 变化保留各自的展示方式。窄容器中卡片堆叠排列。能力标签区分文件／环境、搜索、技能、人机交互和交付物。未知工具保留记录中的名称与通用分类。

关键成员行保持可见，原始调度事件折叠在下方。Workflow 线段按成员起止事件顺序对齐，不按耗时缩放。完整配对可证明观测到的重叠或先后执行，不能证明依赖或等待屏障。Agent 行展示父会话观察到的派发与回执；回执送达不能证明执行结束时间或并发关系。后续消息、结束通知和控制操作在原始位置保留为简短回执，提供“定位调度”和“返回回执”。缺失来源会明确标注。Todo 比较整份列表的数量，不推断任务标识。选择生命周期端点或原始卡片可检查原始记录。

展开已记录的子任务，可在父请求内的完整调度块下方阅读子历史。展开通过认证的子会话地址跟随历史，不选择或恢复其 Agent；收起只释放视图观察，不取消工作。子会话的分页和失败独立处理。加载到已记录的种子边界时，fork 继承内容会明确标记。视图展示记录证据，不验证 Agent 的结论。
