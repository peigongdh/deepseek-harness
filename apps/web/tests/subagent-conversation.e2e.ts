...
describe('web e2e: persisted subagent conversation and human continuation', () => {
  ...
  it('expands child evidence inside the enhanced parent trajectory and inspects its raw records', async () => {
    ...
    const summaryBounds = await enhanced.locator('[data-scheduling-block="agents"]').first().boundingBox()
    const childBounds = await childCard.boundingBox()
    expect(childBounds!.y).toBeGreaterThanOrEqual(summaryBounds!.y + summaryBounds!.height)
    ...
    const receipt = enhanced.locator('[data-scheduling-receipt]').last()
    await receipt.getByRole('button', { name: 'Locate dispatch', exact: true }).click()
    await enhanced.getByRole('button', { name: 'Return to receipt', exact: true }).click()
    await expect.poll(() => receipt.locator('[data-enhanced-record]').getAttribute('aria-pressed')).toBe('true')
    const receiptBounds = await receipt.boundingBox()
    expect(receiptBounds!.y).toBeGreaterThanOrEqual(0)
